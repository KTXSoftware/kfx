...
extern bool isVertexShader(std::string from);

int compileHLSLToD3D11(const char* from, const char* to, const std::map<std::string, int>& attributes) {
	...
	HRESULT hr = D3DCompile(data, length, from, nullptr, nullptr, "main", isVertexShader(from) ? "vs_4_0" : "ps_4_0", /*D3DCOMPILE_DEBUG |*/ D3DCOMPILE_ENABLE_BACKWARDS_COMPATIBILITY, 0, &shaderBuffer, &errorMessage);
	if (hr != S_OK) hr = D3DCompile(data, length, from, nullptr, nullptr, "main", isVertexShader(from) ? "vs_5_0" : "ps_5_0", /*D3DCOMPILE_DEBUG |*/ D3DCOMPILE_ENABLE_BACKWARDS_COMPATIBILITY, 0, &shaderBuffer, &errorMessage);
	if (hr == S_OK) {
